...
import { createAsciiAtmosphereConfig, initialTrailSettings, type AsciiAtmosphereConfig, type GlyphBand, type TrailSettings } from '../asciiAtmosphereConfig'
...
export type AtmosphereSettingsController = {
  ...
  setConfigNumber: (section: NumericConfigSection, key: string, value: number) => void
  setGridFont: (font: string) => void
  setGlyphSetting: (index: number, key: GlyphSetting, value: number | string) => void
  randomizeFluid: () => void
...
export function useAtmosphereSettings(): AtmosphereSettingsController {
  ...
  function setConfigNumber(section: NumericConfigSection, key: string, value: number) {
  ...
  function setGlyphSetting(index: number, key: GlyphSetting, value: number | string) {
    setAtmosphereConfig((current) => ({
      ...current,
  ...
  function randomizeFluid() {
    setAtmosphereConfig((current) => ({ ...current, fluid: randomizeFluidConfig(current.fluid) }))
  }
