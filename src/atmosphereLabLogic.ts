...
export function randomizeFluidConfig(fluid: Record<string, number>, random = Math.random) {
  return Object.fromEntries(Object.entries(fluid).map(([key, currentValue]) => {
    if (!fluidLabControls.some((control) => control.key === key)) return [key, currentValue]
